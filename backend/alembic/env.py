...
def ensure_database_exists():
    ...
    engine = create_engine(db_url.rsplit("/", 1)[0])  # Conecta al servidor sin especificar la base de datos
    db_name = Config.MYSQL_DB

    try:
        with engine.connect() as connection:
            connection.execute(text(f"CREATE DATABASE IF NOT EXISTS {db_name}"))
            print(f"La base de datos '{db_name}' ha sido creada o ya existía.")
    except OperationalError as e:
        print(f"Error al conectar con el servidor MySQL: {e}")
        raise

# Llama a la función antes de aplicar las migraciones
